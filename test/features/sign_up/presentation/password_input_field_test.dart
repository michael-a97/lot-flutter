  ...
  group('$PasswordInputField', () {
    ...
    Future<void> pumpWidgetUnderTest(WidgetTester tester) async {
      await tester.pumpApp(
        BlocProvider.value(
          value: signUpCubit,
          child: const Material(child: PasswordInputField()),
        ),
      );
    }
    ...
    testWidgets('should call onPasswordChanged when text changes', (
      tester,
    ) async {
      const signUpState = SignUpState();
      when(() => signUpCubit.state).thenReturn(signUpState);
      ...
      verify(() => signUpCubit.onPasswordChanged('new password')).called(1);
    });
